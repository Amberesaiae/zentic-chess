type VoiceCallbacks = {
  ...
  onToolCall: (name: string, input: Record<string, unknown>) => unknown;
};

export type VoiceSession = { close: () => void };

export async function connectRealtimeVoice(callbacks: VoiceCallbacks): Promise<VoiceSession> {
  ...
  channel.addEventListener("message", (event) => {
    ...
    if (message.type === "response.function_call_arguments.done" && message.name && message.call_id) {
      try {
        const output = callbacks.onToolCall(message.name, JSON.parse(message.arguments ?? "{}") as Record<string, unknown>);
  ...
  return { close: () => { stream.getTracks().forEach((track) => track.stop()); channel.close(); peer.close(); audio.srcObject = null; } };
}
