...
export function MatchHeader({ state, onFlip, onReset, onLobby }: { state: MatchState; onFlip: () => void; onReset: () => void; onLobby: () => void }) {
  ...
  return <header className="match-header">
    <div className="wordmark"><span aria-hidden="true">Z</span><b>ENTIC</b></div>
    <div className="game-identity"><button type="button" onClick={onLobby} title="Return to game setup" aria-label="Current game mode: click to change">{matchName} <CaretDown size={13} weight="bold" /></button><span>{state.timeControl === "untimed" ? "No clock" : state.timeControl === "rapid_10" ? "10 min" : "5 min"}</span></div>
    <div className="turn-status" aria-live="polite"><Flag size={16} weight="fill" /><strong>{turnLabel}</strong></div>
    <div className="match-actions">
      <Button className="icon-button" variant="outline" size="icon" onClick={onFlip} aria-label="Rotate board" title="Rotate board"><Swap size={18} /></Button>
      <Button className="lobby-button" variant="outline" onClick={onLobby}><ArrowLeft size={17} /> Game setup</Button>
      <Button onClick={onReset}><ArrowCounterClockwise size={17} /> Start over</Button>
    </div>
  </header>;
}
