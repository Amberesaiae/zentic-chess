...
export function Lobby({ onStart }: { onStart: (config: MatchConfig) => void }) {
  ...
  return <main className="lobby-shell">
    <header className="lobby-header"><div className="wordmark"><span aria-hidden="true">Z</span><b>ZENTIC</b><i /> <small>CHESS, SHARED WITH AGENTS</small></div><span className="lobby-version">A slower way to play online</span></header>
    <section className="lobby-grid">
      <LobbyIntro />
      <form className="setup-panel" onSubmit={(event) => { event.preventDefault(); onStart({ mode, humanColor, difficulty, timeControl, sessionPolicy }); }}>
